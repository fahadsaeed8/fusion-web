    ...
    <section className="w-full bg-black py-6 md:py-[102px] lg:py-[66px] xl:py-[88px]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-6 md:gap-[18px] lg:gap-[30px] xl:gap-10">
          {/* First Row: Title/Description + Cards area */}
          <div className="grid grid-cols-1 md:grid-cols-[1fr_2.5fr] gap-6 md:gap-[18px] lg:gap-[30px] xl:gap-10">
            ...
            {/* Right Side - Healthcare on its own row only */}
            <div className="flex flex-col gap-6 md:gap-[18px] lg:gap-[30px] xl:gap-10 w-full">
              {/* Healthcare - full-width block, no elements beside it, a bit wider (taller) */}
              {topRowIndustries
                .filter((i) => i.name === 'Healthcare')
                .map((industry, index) => (
                  <motion.div
                    key={industry.name}
                    initial={{ opacity: 0, y: 30 }}
                    ...
                    className="block w-full group transition-all ease-in-out duration-400 relative h-[160px] md:h-[180px] lg:h-[220px] xl:h-[280px] rounded-2xl md:rounded-[13px] lg:rounded-[23px] xl:rounded-[30px] overflow-hidden cursor-pointer"
                  >
                    <div
          ...
          {/* Education & Retail – full-width row (like Gaming), colSpan from data, centered in middle */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 md:gap-[18px] lg:gap-[30px] xl:gap-10 md:max-w-4xl md:mx-auto w-full">
            {topRowIndustries
              .filter((i) => i.name !== 'Healthcare')
              .sort((a, b) => {
                const order = { left: 0, middle: 1, right: 2 };
              ...
              .map((industry, index) => {
                const colSpanClass = (industry as { colSpan?: number }).colSpan === 2 ? 'md:col-span-2' : 'md:col-span-1';
                return (
                  <motion.div
                    key={industry.name}
                    initial={{ opacity: 0, y: 30 }}
                    ...
                    className={`${colSpanClass} block w-full group transition-all ease-in-out duration-400 relative h-[138px] md:h-[158px] lg:h-[199px] xl:h-[265px] rounded-2xl md:rounded-[13px] lg:rounded-[23px] xl:rounded-[30px] overflow-hidden cursor-pointer`}
                  >
                    <div
                      className="absolute inset-0 bg-cover bg-center transition-all ease-in-out duration-400 opacity-40 group-hover:scale-110 group-hover:opacity-80"
                      style={{ backgroundImage: industry.bgImage }}
